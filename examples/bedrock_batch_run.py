...
prompts_df = pd.concat([prompts_df] * 100)
prompts_df["recordId"] = [str(x) for x in list(range(100))]
...
if not os.path.exists(input_file_path):
    ...
    results = client.create_batch_job(prompts_df, s3_input_path, s3_output_path, model=model_name,  job_name=job_name)
